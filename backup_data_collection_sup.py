...
(season_subset(df85_15,2014).groupby("PLAYER_NAME")
                            .apply(lambda x: pd.DataFrame(map(lambda y: calc_ngame_avg(x.sort_values("GAME_DATE"),["AST","FANTASY_PTS"],y,5),x.GAME_DATE))))
...
kobe_df = (df85_15[df85_15.PLAYER_NAME == "Kobe Bryant"].groupby(["PLAYER_NAME","SEASON_ID"])
                            .apply(lambda x: pd.DataFrame(map(lambda y: calc_ngame_avg(x.sort_values("GAME_DATE"),["AST","REB","PTS","OPP_ELO","TOV","STL","BLK"],y,5),x.GAME_DATE)))).reset_index().drop('level_2',axis = 1)
...
df = kobe_dftest[(kobe_dftest.SEASON_ID <= 220011) & (kobe_dftest.SEASON_ID >= 2006)]
from sklearn.cross_validation import train_test_split
itrain, itest = (df.SEASON_ID < 22011), (df.SEASON_ID == 22011) #train_test_split(xrange(df.shape[0]), train_size=0.7)
...
STANDARDIZABLE = [u'5D_AST', u'5D_PTS', u'5D_REB', '5D_OPP_ELO','5D_TOV',u'FANTASY_PTS']
...
lcols = dftouse[STANDARDIZABLE].columns.tolist()
lcols.remove('FANTASY_PTS')
...
Yresp=dftouse['FANTASY_PTS'].values 
